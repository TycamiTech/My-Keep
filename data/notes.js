...
Happy note-taking!`
  },
  {
    ...
    date: "2026-02-05",
    category: "Server",
    content: `Repository Debian adalah pusat penyimpanan digital (gudang data) yang memuat kumpulan paket perangkat lunak (.deb), kode sumber, dan metadata yang terorganisir untuk sistem operasi Debian.
## Debian Utama
\`\`\`
...
\`\`\``
  },
  {
    ...
    date: "2026-02-06",
    category: "Server",
    content: `1. \x60apt install apache2\x60
2. \x60systemctl status apache2\x60
3. \x60nano /etc/apache2/sites-available/tycami.xyz.conf\x60

\x60\x60\x60apache
<VirtualHost 192.168.5.100:80> 
  ServerName tycami.xyz
  ServerAlias www.tycami.xyz

  DocumentRoot /var/www/tycami.xyz/public_html 

  ErrorLog \${APACHE_LOG_DIR}/tycami.xyz-error.log
  CustomLog \${APACHE_LOG_DIR}/tycami.xyz-access.log combined
</VirtualHost> 
\x60\x60\x60
...
SELESAIII`
  },
  {
    ...
    date: "2026-02-06",
    category: "Server",
    content: `1. \x60apt install bind9\x60
2. \x60cd /etc/bind\x60 (untuk pindah repositori)
3. \x60ls\x60 (untuk menampilkan isi repositori)

4. \x60nano named.conf.default-zones\x60
\x60\x60\x60bash
zone localhost menjadi domain name
zone 127 menjadi - .168.192 (3 oktet dari IP Debian)
db.127 menjadi db. - (oktet 4 dari IP Debian)
\x60\x60\x60

5. buat file db.oktet 4
- \x60cp db.127 db.IP oktet 4\x60

6. \x60nano db.local\x60 (konfigurasi)
- ctrl w, ctrl r
- ubah/replace local.host menjadi domain name
- Konfigurasi IP 127... menjadi IP Debian(normal)
- ctrl k di bagian @ IP Debian
- ctrl u 3x
- ubah @ menjadi www

7. \x60nano db.ip oktet 4\x60
- ctrl w, ctrl r
- ubah/replace local.host menjadi nama domain
- All
- ubah @ menjadi IP oktet 4
- ctrl k di bagian IP oktet 4
- ctrl u 3x

8. \x60systemctl restart bind9\x60
- \x60nslookup nama domain\x60
- \x60nslookup IP debian\x60


**NOTE : UNINSTALL Paket**
\x60\x60\x60bash
apt purge bind9
apt purge apache2
apt delete bind9
...
SELESAIII`
  },
  {
    ...
    date: "2026-02-06",
    category: "Network",
    content: `Konfigurasi UKK MikroTik dengan pembagian interface dan aturan firewall.

## Sistem Login
- **Username**: admin
- **Password**: 00123

## Konfigurasi Interface & IP
- **Eth 1**: Internet (DHCP Client/Static sesuai ISP)
- **Eth 2**: 192.168.10.1/24 (**Guru**)
- **Eth 3**: 192.168.20.1/24 (**Murid**)
- **Eth 4**: 192.168.30.1/24 (**Server**)

## Layanan DHCP
- Aktifkan DHCP Server pada **Eth 2** (Guru) dan **Eth 3** (Murid).

## Manajemen Akses Admin
- **Login User Admin**: Hanya diizinkan dari segmen IP **Server & Guru**.
- **Interface List**: Buat list "Admin" yang hanya berisi **Eth 4**.
- **Neighbor List**: Hanya aktif pada list "Admin".
- **Tools/MAC Server**: Batasi akses Discovery hanya untuk list "Admin".

## Konfigurasi Port Service
Ubah port default untuk keamanan:
| Service | Port Baru |
|---------|-----------|
| Winbox  | 8299      |
| SSH      | 222       |
...
## Firewall Rules
1. **Drop Siswa ke Guru**: Blokir akses dari network Murid (Eth 3) ke network Guru (Eth 2).
2. **Mengamankan DNS**: Batasi akses DNS port 53 (UDP/TCP) pada Ether 3 agar tidak disalahgunakan.
3. **Accept Server ke Guru**: Izinkan akses penuh dari network Server ke network Guru.
4. **Blok Brute Force**: Tambahkan rule untuk mendeteksi dan memblokir upaya login paksa.
5. **Log**: Aktifkan logging untuk setiap Rule Firewall yang kritikal untuk monitoring.`
  }
];
